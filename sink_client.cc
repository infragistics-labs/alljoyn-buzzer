...
class MySinkListener : public SinkListener {
    void SinkAdded(const char* name) {
        printf("Sink appeared: %s\n", name);
        if (broadcastTo == NULL || strcmp(broadcastTo, name) == 0) {
            printf("SinkAdded: %s\n", name);
...
int main(int argc, char** argv, char** envArg) {
    if (argc < 2 || argc > 3) {
        printf("Usage: %s file.wav [friendlyname]\n", argv[0]);
        return 1;
    }
    ...
    /* Create message bus */
    BusAttachment* msgBus = new BusAttachment("hackweek-buzzer-client", true);
    if (!msgBus) {
    	printf("FAILED\n");
    } 
