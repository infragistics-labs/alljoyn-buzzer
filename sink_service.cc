...
class AboutStore : public PropertyStore {
  public:
    ...
    QStatus ReadAll(const char* languageTag, PropertyStore::Filter filter, MsgArg& all) {
        ...
        props[3].Set("{sv}", "DeviceId", new MsgArg("s", mFriendlyName));
        props[4].Set("{sv}", "AppName", new MsgArg("s", "Buzzer-Audio"));
        ...
        if (PropertyStore::READ == filter) {
            ...
            props[8].Set("{sv}", "Description", new MsgArg("s", "Infragistics Buzzer Audio"));
...
int main(int argc, char** argv, char** envArg) {
    ...
    const char* friendlyName = "hackweek-buzzer-audio";
